...
use crate::{
    QAbstractSocket, QIODevice, QIODeviceOpenMode, QSocketAddr, QSslCertificate,
    QSslImplementedClass, QSslSslProtocol, QSslSupportedFeature, QTcpSocket,
};
...
mod ffi {
    ...
    unsafe extern "C++Qt" {
        ...
        #[Self = "QSslSocket"]
        #[rust_name = "active_backend"]
        fn activeBackend() -> QString;
        ...
        #[Self = "QSslSocket"]
        #[rust_name = "available_backends"]
        fn availableBackends() -> QList_QString;

        #[doc(hidden)]
        #[Self = "QSslSocket"]
        #[rust_name = "ssl_library_build_version_string_or_empty"]
        fn sslLibraryBuildVersionString() -> QString;

        #[doc(hidden)]
        #[Self = "QSslSocket"]
        #[rust_name = "ssl_library_version_string_or_empty"]
        fn sslLibraryVersionString() -> QString;
...
impl QSslSocket {
    ...
    ///
    /// If the SSL handshake is successful, `QSslSocket` emits [`encrypted`](QSslSocket::encrypted).
    pub fn connect_to_host_encrypted<A>(self: Pin<&mut Self>, addr: A, mode: QIODeviceOpenMode)
    where
        A: Into<QSocketAddr>,
    {
        let addr = addr.into();
    ...
    pub fn ssl_library_build_version_string() -> Option<QString> {
        Self::ssl_library_build_version_string_or_empty().nonnull()
    }
