...
mod ffi {
    ...
    enum QNetworkRequestAttribute {
        ...
        /// Requests only, type: `QMetaType::Bool` (default: false) Indicates whether the Network Access API should automatically follow a HTTP redirect response or not. Currently redirects that are insecure, that is redirecting from "https" to "http" protocol, are not allowed.
        ...
        /// Requests only, type: `QMetaType::Bool` (default: false) If set, this attribute will force `QNetworkAccessManager` to use HTTP/2 protocol without initial HTTP/2 protocol negotiation. Use of this attribute implies prior knowledge that a particular server supports HTTP/2. The attribute works with SSL or with 'cleartext' HTTP/2 if Http2CleartextAllowedAttribute is set. If a server turns out to not support HTTP/2, when HTTP/2 direct was specified, `QNetworkAccessManager` gives up, without attempting to fall back to HTTP/1.1. If both Http2AllowedAttribute and Http2DirectAttribute are set, Http2DirectAttribute takes priority.
        Http2DirectAttribute,
        #[doc(hidden)]
        ResourceTypeAttribute, // internal
        /// Requests only, type: `QMetaType::Bool` (default: false) If set, this attribute will make `QNetworkAccessManager` delete the `QNetworkReply` after having emitted "finished".
