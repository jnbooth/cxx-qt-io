...
mod ffi {
    ...
    enum QNetworkRequestAttribute {
        ...
        /// Requests only, type: `QMetaType::String` Holds the full local server name to be used for the underlying `QLocalSocket`. This attribute is used by the [`QNetworkAccessManager`](https://doc.qt.io/qt-6/qnetworkaccessmanager.html) to connect to a specific local server, when `QLocalSocket`'s behavior for a simple name isn't enough. The URL in the `QNetworkRequest` must still use unix+http: or local+http: scheme. And the hostname in the URL will be used for the Host header in the HTTP request.
