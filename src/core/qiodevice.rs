...
mod ffi {
    ...
    unsafe extern "C++Qt" {
        ...
        /// Reads at most `max_size` bytes from the device into `data`, and returns the number of bytes read. If an error occurs, such as when attempting to read from a device opened in `WriteOnly` mode, this function returns -1.
        ///
        ...
        ///
        /// Note that on sequential devices, data may not be immediately available, which may result in a partial line being returned. By calling the `can_read_line()` function before reading, you can check whether a complete line (including the newline character) can be read.
        ///
        ///
        /// # Safety
        ///
        ...
        ///
        /// Restores the input stream to the point of the `start_transaction()` call. This function is commonly used to rollback the transaction when an incomplete read was detected prior to committing the transaction.
        ...
        /// Sets the current read channel of the device to the given channel. The current input channel is used by the functions `read()`, `read_all()`, `read_line()`, and `get_char()`. It also determines which channel triggers the device to emit `ready_read()`.
        #[rust_name = "set_current_read_channel"]
        fn setCurrentReadChannel(self: Pin<&mut QIODevice>, channel: i32);

        /// Sets the current write channel of the device to the given channel. The current output channel is used by the functions `write()` and `put_char()`. It also determines which channel triggers the device to emit `bytes_written()`.
        ...
        /// For open random-access devices, this function returns the size of the device. For open sequential devices, `bytes_available()` is returned.
        ///
        ...
        /// # Safety
        ///
        /// `c` must be the value that was retrieved by [`QIODevice::get_char`] immediately prior
        ...
        ///
        /// `bytes_written()` is not emitted recursively; if you reenter the event loop or call `wait_for_bytes_written()` inside a slot connected to the `bytes_written()` signal, the signal will not be reemitted (although `wait_for_bytes_written()` may still return true).
        #[qsignal]
        #[rust_name = "bytes_written"]
        fn bytesWritten(self: Pin<&mut QIODevice>, bytes: i64);

        /// This signal is emitted every time a payload of data has been written to the device. The bytes argument is set to the number of bytes that were written in this payload, while channel is the channel they were written to. Unlike ``bytes_written()`, it is emitted regardless of the current write channel.
        ///
        /// `channel_bytes_written()` can be emitted recursively - even for the same channel.
        #[qsignal]
        #[rust_name = "channel_bytes_written"]
        fn channelBytesWritten(self: Pin<&mut QIODevice>, channel: i32, bytes: i64);

        /// This signal is emitted when new data is available for reading from the device. The channel argument is set to the index of the read channel on which the data has arrived. Unlike `read_read()`, it is emitted regardless of the current read channel.
        ///
        /// `channel_ready_read()` can be emitted recursively - even for the same channel.
        #[qsignal]
        ...
        ///
        /// `ready_read()` is not emitted recursively; if you reenter the event loop or call `wait_for_read_read()` inside a slot connected to the `ready_read()` signal, the signal will not be reemitted (although `wait_for_ready_read()` may still return `true`).
        #[qsignal]
...
impl QIODevice {
    ...
    /// Returns `true` if the device is open; otherwise returns `false`. A device is open if it can be read from and/or written to. By default, this function returns `false` if `open_mode()` returns `NotOpen`.
    pub fn is_open(&self) -> bool {
        ffi::qiodevice_is_open(self)
    }

    /// Returns `true` if data can be read from the device; otherwise returns `false`. Use `bytes_available()` to determine how many bytes can be read.
    ...
    /// Reads at most `data.len()` bytes from the device into `data`, without side effects (i.e., if you call `read()` after `peek()`, you will get the same data). Returns the number of bytes read. If an error occurs, such as when attempting to peek a device opened in `WriteOnly` mode, this function returns -1.
    ///
    /// 0 is returned when no more data is available for reading.
    ...
    /// Reads at most `data.len()` bytes from the device into `data`, and returns the number of bytes read. If an error occurs, such as when attempting to read from a device opened in `WriteOnly` mode, this function returns -1.
    ///
    /// 0 is returned when no more data is available for reading. However, reading past the end of the stream is considered an error, so this function returns -1 in those cases (that is, reading on a closed socket or after a process has died).
