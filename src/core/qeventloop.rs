...
mod ffi {
    ...
    extern "C++" {
        include!("cxx-qt-io/qeventloop.h");
    ...
    unsafe extern "C++Qt" {
        ...
        #[cfg(cxxqt_qt_version_at_least_6_7)]
        #[doc(hidden)]
        #[rust_name = "process_events_until_deadline"]
        fn processEvents(
...
impl QEventLoop {
        ...
        T: Into<QDeadlineTimer>,
    {
        fn inner(
            ...
            #[cfg(cxxqt_qt_version_at_least_6_7)]
            event_loop.process_events_until_deadline(flags, deadline);
            #[cfg(not(cxxqt_qt_version_at_least_6_7))]
            event_loop.process_events_until_msecs(flags, crate::util::MSecs::msecs(&deadline));
        }
        inner(self, flags, deadline.into());
    }

    /// Process pending events until `deadline` has expired, or until there are no more events to process, whichever happens first. `deadline` can be a [`QDeadlineTimer`](crate::QDeadlineTimer) or [`Duration`](std::time::Duration).
    ///
    /// **Note:** This function does not process events continuously; it returns after all available events are processed.
    pub fn process_all_events_until<T>(self: Pin<&mut QEventLoop>, deadline: T)
    where
        T: Into<QDeadlineTimer>,
    {
        self.process_events_until(
            QEventLoopProcessEventsFlag::AllEvents.into(),
            deadline.into(),
        );
    }
}
